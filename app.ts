...
function newsFeed(): void {
    let newsFeedData: NewsFeed[] = store.feeds; //getData(NEWS_URL);
    ...
    if(newsFeedData.length === 0) {
        newsFeedData = store.feeds = makeFeeds(getData<NewsFeed[]>(NEWS_URL));
    }
    ...
    template = template.replace('{{__next_page__}}', String(store.lastPage > store.currentPage ? store.currentPage + 1 : store.lastPage));
